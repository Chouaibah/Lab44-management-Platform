...
export default function InstructorVMView() {
  ...
  const handleConvertToTemplate = async () => {
    const { vmUuid, vmName } = convertDialog;
    setConvertLoading(vmUuid);
    try {
      const res = await fetch(`/api/xapi/vms/${vmUuid}/action`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'convert-to-template', vmName: convertTemplateName || vmName }),
      });
      const data = await res.json();
      if (res.ok) {
        toast.success(`VM "${convertTemplateName || vmName}" converted to template`);
        setConvertDialog({ open: false, vmUuid: '', vmName: '' });
        refreshData();
      } else {
        toast.error(data.error || 'Failed to convert VM');
      }
    } catch {
      ...
      <Dialog open={convertDialog.open} onOpenChange={(open) => { if (!open) setConvertDialog({ open: false, vmUuid: '', vmName: '' }); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter a name for the template. The VM will be converted and removed from this list.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setConvertTemplateName(e.target.value)}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setConvertDialog({ open: false, vmUuid: '', vmName: '' })}>Cancel</Button>
            <Button onClick={handleConvertToTemplate} disabled={!convertTemplateName.trim() || convertLoading === convertDialog.vmUuid}>
              {convertLoading === convertDialog.vmUuid ? 'Converting...' : 'Convert'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
