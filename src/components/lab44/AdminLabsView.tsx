...
export default function AdminLabsView() {
  ...
  const renderFormDialog = (isOpen: boolean, onOpenChange: (open: boolean) => void, onSubmit: () => void, title: string, isEdit: boolean) => (
    ...
    <FlaskConical className="h-5 w-5" /> {title}
    </DialogTitle>
    <DialogDescription>
    ...
    <div>
    <Label className="text-xs font-medium">Auto-Approve Students</Label>
    <p className="text-[11px] text-muted-foreground">Students can join without admin approval</p>
    </div>
    <Switch
  ...
  return (
    ...
    <FlaskConical className="h-4 w-4" /> All Labs
    <Badge variant="secondary" className="h-5 text-[10px] px-1.5">{filteredLabs.length}</Badge>
    </CardTitle>
    </CardHeader>
    ...
    <CardHeader className="pb-3">
    <CardTitle className="text-base flex items-center gap-2">
    <GraduationCap className="h-4 w-4" /> Study Levels
    <Badge variant="secondary" className="h-5 text-[10px] px-1.5">{levels.length}</Badge>
    </CardTitle>
    </CardHeader>
    <CardContent>
    ...
    {levels.length === 0 ? (
      ...
      {levels.map((level) => (
        ...
        <div>
        <p className="font-medium text-sm">{level.year} - {level.specialty}</p>
        </div>
