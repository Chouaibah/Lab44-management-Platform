...
export default function StudentChoiceView() {
        ...
        try {
          ...
          if (myLabIds.length > 0) {
            ...
            for (const labId of myLabIds) {
              ...
              if (Array.isArray(rlData)) allLinks.push(...rlData);
  ...
  {
    view: 'student-announcements' as AppView,
    icon: <Megaphone className="h-5 w-5" />,
    iconBg: 'bg-amber-100 dark:bg-amber-900/30',
    iconColor: 'text-amber-600 dark:text-amber-400',
    title: 'Announcements',
    desc: 'View announcements, pinned messages, and stay updated',
  },
];
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:py-16">
      ...
      <motion.div {...fadeSlide} transition={{ delay: 0.05 }} className="text-center mb-10">
        ...
        >
          <span className="inline-flex items-center gap-1.5">{greeting.icon} {greeting.text}</span>
        </motion.p>
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-3xl sm:text-4xl font-bold mb-2">
          <span className="text-primary font-semibold">{name}</span>
        </motion.h1>
        <p className="text-muted-foreground mt-1">Choose what you&apos;d like to do today</p>
      </motion.div>
      ...
      {/* Lab Materials Empty State - show when no materials but student has joined labs */}
